...
public:
	...
	size_t ComputeBeliefs(T* lambdaBase, T epsilon, T* belPtr, bool OnlyUnaries, int num_pair_vals) {
        T* mem = belPtr;
        entropy = 0;
        //for (int i = 0; i < num_pair_vals; i++)
        //{
        //    region_belief_sums[i] = 0.0;
        //}
		for (typename std::vector<MPNode*>::const_iterator r = Graph.begin(), r_e = Graph.end(); r != r_e; ++r) {
			MPNode* r_ptr = *r;
			size_t numVars = r_ptr->varIX.size();
	...
	T getBelief(S region_num, S val_num) {
		T result = *((T*)(Graph[region_num]->belief + ((size_t)val_num)));
 		return result;
	}
...
public:
	...
	T RunMP(T* lambdaGlobal, T epsilon, int num_iters) {
		size_t msgSize = g.GetLambdaSize();
		if (msgSize == 0) {
			typename MPGraph<T, S>::DualWorkspaceID dw = g.AllocateDualWorkspaceMem(epsilon);
			//std::cout << "0: " << g.ComputeDual(NULL, epsilon, dw) << std::endl;
