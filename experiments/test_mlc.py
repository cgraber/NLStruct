from . import mlc_experiment as mlc
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description = 'Run experiment for bibtex data')
    parser.add_argument('dataset', choices=['bibtex', 'bookmarks', 'delicious'])
    parser.add_argument('model', choices=['unary', 'pairwise', 'global_mlp'])
    ...
    if args.dataset == 'bibtex':
        train_data = mlc.BibtexDataset(mlc.FULL, args.train_feature_file, args.train_label_file)
        test_data = mlc.BibtexDataset(mlc.FULL, args.test_feature_file, args.test_label_file)
        num_labels = 159
        num_features = 1836
        ...
        train_data = mlc.DeliciousDataset(mlc.FULL, args.train_feature_file, args.train_label_file)
        test_data = mlc.DeliciousDataset(mlc.FULL, args.test_feature_file, args.test_label_file)
        num_features = 500
        ...
        train_data = mlc.BookmarksDataset(mlc.TRAIN, args.train_feature_file, args.train_label_file)
        val_data = mlc.BookmarksDataset(mlc.VAL, args.train_feature_file, args.train_label_file)
        test_data = mlc.BookmarksDataset(mlc.TEST, args.train_feature_file, args.train_label_file)
        num_features = 2150
    ...
    args_dict['unary_hidden_layers'] = args.unary_hidden_layers
    args_dict['unary_hidden_size'] = args.unary_hidden_size
    if args.model == 'unary':
        args_dict['pair_inds'] = {}
        graph = Graph(nodes, [], 2, mlc.MLCPotentialModel, args_dict, False)
        model = PairwiseModel([graph], len(nodes), 2, params)
        graph.potential_model.unary_model.load_state_dict(torch.load(args.pretrain))
    elif args.model == 'pairwise':
        full_graph = Graph(nodes, pairs, 2, mlc.MLCPotentialModel, args_dict, False)
        model = PairwiseModel([full_graph], len(nodes), 2, params)
        model.load(args.pretrain)
    elif args.model == 'global_mlp':
        if args.use_residual:
            params['global_model'] = mlc.GlobalModel_Res
        else:
            if args.mlp_init is None:
                params['global_model'] = build_mlp_global_model
            elif args.mlp_init == 'v1':
                params['global_model'] = mlc.build_initialized_mlp_global_model_v1
            elif args.mlp_init == 'v2':
                params['global_model'] = mlc.build_initialized_mlp_global_model_v2

        graphs = []
        nodes_graph = Graph(nodes, [], 2, mlc.MLCPotentialModel, args_dict, False)
        graphs.append(nodes_graph)
        for ind,pair in enumerate(pairs):
            graphs.append(Graph([], [pair], 2, mlc.MLCPotentialModel, args_dict, False))
        if args.interleave:
        ...
        #nodes_graph.potential_model.unary_model.load_state_dict(torch.load(args.unary_model))
    ...
    train_p, train_r, train_f1 = mlc.calculate_accuracy(train_data, train_results)

    if args.dataset == 'bookmarks':
        val_results = model.test(val_data, params)
        val_p, val_r, val_f1 = mlc.calculate_accuracy(val_data, val_results)

    print("Testing on test data...")
    test_results = model.test(test_data, params)
    test_p, test_r, test_f1 = mlc.calculate_accuracy(test_data, test_results)
